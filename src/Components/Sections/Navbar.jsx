...
const menuItems = [
  ...
  { name: 'Subscription', href: '#subcriptionplan' },
...
const Navbar = () => {
  ...
  return (
    <>
      <nav className="z-10 sticky top-[34px] left-0 w-full text-white py-2 px-4 md:px-8 bg-black">
            ...
            <div className="space-y-4 font-[PoppinsMedium] flex flex-col">
              {menuItems.map((item, index) => (
                <div key={index} className='w-full'>
                  <a onClick={toggleMenu} href={item.href} className="hover:text-white text-[14px] w-full">{item.name}</a>
                </div>
              ))}
            </div>
          </div>
